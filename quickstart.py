...
def get_attachments(mail_service, user_id, emails_id):
    ...
    try:
        ...
        for ids in emails_id:
            mail_data = (mail_service.users().messages().get(userId=user_id, id=ids, format='full').execute())
        ...
        return {'data': attachment_data, 'filename': filename, 'as_bytes': attachment_data_as_bytes}
...
def main():
    ...
    # ########################
    # TODO Make this part as a function
    # Check for any emails matching query:
    query = ['trzyfaktury']
    matches = []
    emails_id = []
    for items in query:
        matches.append(list_messages_matching_query(mail_service, 'me', items))
    ...
    # file_data = get_attachments(mail_service, 'me', emails_id)
